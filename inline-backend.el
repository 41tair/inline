...
(defun inline--responses-content-text (content)
  ...
  (let ((parts nil)
        (items (if (vectorp content) (append content nil) content)))
    (dolist (item items)
...
(defun inline--responses-output-text (data)
  ...
  (or
   ...
   (let* ((parts nil)
          ...
          (items (if (vectorp output) (append output nil) output)))
     (dolist (item items)
  ...
  (condition-case _
      (let* ((json-object-type 'alist)
        ...
        (or
         ...
         (when-let ((choice (cond ((vectorp choices) (aref choices 0))
                                  ((listp choices) (car choices)))))
           (let* ((message (alist-get 'message choice))
